...
Как уже было указано выше, пока эти записи будут оставлены без внимания. Основной целью является извлечение данных, полученных с подключенных к АЦП датчиков, поэтому посмотрим последний фрагмент а архиве:
...
Второй и третий с конца фрагменты являются идентичными последнему, из чего можно сделать вывод, что внутренняя память прибора заполнена не до конца, но создаваемый архив имеет фиксированный размер, поэтому свободное место заполняется значениями `0xff`. Проведем поиск по архиву с конца такого фрагмента, который бы отличался от фрагмента, полностю заполненного значениями `0xff`. В рассматриваемом архиве такой фрагмент оказался под номером 228644. 

```
a52a 67ec 485e 0000 f843 dac3 2943 efe6 ce43 f612 5201 f612 5201 f612 5201 f612
5201 f612 5201 f612 52df a52a 68ec 485e 0000 f843 dabc d743 efe0 6843 f60a f701
f60a f701 f60a f701 f60a f701 f60a f701 f60a f715 a52a 6aec 485e 0000 f843 dab7
8e43 efd1 1d43 f601 3601 f601 3601 f601 3601 f601 3601 f601 3601 f601 3697 a52a
6bec 485e 0000 f843 dab3 9b43 efe1 cc43 f60a e501 f60a e501 f60a e501 f60a e501
f60a e501 f60a e568 a52a 6cec 485e 0000 f843 dab5 ff43 efda 8243 f5fe 0c01 f5fe
0c01 f5fe 0c01 f5fe 0c01 f5fe 0c01 f5fe 0c51 ffff ffff ffff ffff ffff ffff ffff
ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff ffff 
```

Здесь так же фрагмент начинается с байта `0xa5`, затем идет длина записи `0x2a` или 42 байта. Смотрим, какой байт находится на позиции 43: опять `0xa5`, означающий начало следующей записи! Разделим фрагмент 228644 на записи:

```
a52a 67ec 485e 0000 f843 dac3 2943 efe6 ce43 f612 5201 f612 5201 f612 5201 f612 5201 f612 5201 f612 52df
a52a 68ec 485e 0000 f843 dabc d743 efe0 6843 f60a f701 f60a f701 f60a f701 f60a f701 f60a f701 f60a f715
a52a 6aec 485e 0000 f843 dab7 8e43 efd1 1d43 f601 3601 f601 3601 f601 3601 f601 3601 f601 3601 f601 3697
a52a 6bec 485e 0000 f843 dab3 9b43 efe1 cc43 f60a e501 f60a e501 f60a e501 f60a e501 f60a e501 f60a e568
a52a 6cec 485e 0000 f843 dab5 ff43 efda 8243 f5fe 0c01 f5fe 0c01 f5fe 0c01 f5fe 0c01 f5fe 0c01 f5fe 0c51
...
1. Обозначение начала записи хранится в 1 байте, его значение - `0xa5`;
2. Длина записи хранится во 2 байте, в приведенном фрагменте его значение составляет `0x2a` или 42;
3. Временная метка содержится в последовательности с 3 по 6 байт. Байты хранятся в обратном порядке (Little-endian);
4. Выход за пределы уставки хранится в 7 байте. В приведенном фрагменте значение этого байта составляет `0xf8` или в двоичном формате `0b11111000`. Прибор содержит 8 каналов, таким образом каждый бит 7-ого байта отвечает за определенный канал. `1` означает выход за границы уставки, `0` - значение параметра находится в пределах уставки;
5. Что-то должно хранится в 8 байте, но что именно - установить не удалось;
6. Ошибки хранятся в 9 байте. В приведенном фрагменте значение этого байта составляет `0xf8` или в двоичном формате `0b11111000`. Прибор содержит 8 каналов, таким образом каждый бит 9-ого байта отвечает за определенный канал. `1` означает ошибку считанных с датчика данных, `0` - отсутсвие ошибки;
7. Считанные с датчиков значения хранятся в последовательности с 10 по 41 байт. Это соответсвует 32 байтам, или по 4 байта на 8 каналов. Байты хранятся в прямом порядке (Big-endian).
8. В последнем, 42-ом байте, вероятно, хранится какая-то контрольная сумма, отвечающая за целостность всей записи. Но я так и не понял, как ее посчитать.
...
5. README.md - определить, в каком порядке читаются биты при определении выхода за пределы уставки
